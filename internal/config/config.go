...
# file, "warn" lets agy cut it and notes that, "fail" refuses it. A reroute keeps the model that was asked for
# (on web, the same family: gemini-3.8-flash-low becomes "3.8 Flash"); when the other backend has no such model
# the request fails rather than run on a different one.
