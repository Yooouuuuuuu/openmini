...
// maxMessageBytes is the size above which agy silently drops the rest of a
// message (verified: a 260,296-byte message reached the model as its first
// 192,000 bytes plus a "<truncated N bytes>" marker).
...
func (a *Agy) Ready() error {
	...
	if !a.readyAt.IsZero() && time.Since(a.readyAt) < time.Minute {
		return a.readyErr // "agy models" spawns a process; a minute-old answer is fine for health checks
	}
	err := a.readyNow()
	a.readyAt, a.readyErr = time.Now(), err
	return err
}

func (a *Agy) readyNow() error {
	lst := exec.Command(a.cfg.Binary, "models")
	backend.PrepareAgy(lst)
	out, err := lst.Output()
	if err != nil {
...
func (a *Agy) Complete(c backend.Call) (backend.Result, error) {
	if over := len(c.Prompt) - maxMessageBytes; over > 0 {
		note := fmt.Sprintf("prompt is %d bytes; agy truncates messages above %d bytes and would drop the last %d bytes", len(c.Prompt), maxMessageBytes, over)
